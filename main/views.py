...
import tempfile
# WeasyPrint es opcional: sin librerías del sistema (gobject/pango/gtk) no debe tumbar el servidor.
# Las vistas PDF muestran fallback en HTML si no está disponible.
try:
    ...
    print(f"[WARN] WeasyPrint no disponible ({e}). PDFs deshabilitados, resto del sitio funcionará.")
...
# ------------------
# PDF - Funciona sin WeasyPrint: muestra HTML si faltan librerías del sistema
...
def generar_pdf_usuarios(request):
    ...
    # Si WeasyPrint no está disponible, mostrar versión HTML en lugar de PDF
    if not WEASYPRINT_AVAILABLE or HTML is None:
        messages.warning(request, "PDF no disponible en este entorno (falta WeasyPrint/GObject). Mostrando vista HTML.")
    ...
    try:
        ...
        messages.warning(request, f"Error generando PDF ({e}). Mostrando vista HTML.")
        return HttpResponse(html_string)
